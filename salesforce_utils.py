...
def load_salesforce_data():
    try:
        ...
        try:
            user_fields = get_safe_user_fields()
            user_query = f"SELECT {', '.join(user_fields)} FROM User WHERE IsActive = true"
            user_url = SF_USERS_URL + quote(user_query)
            user_resp = requests.get(user_url, headers=headers, timeout=30)
            if user_resp.status_code == 200:
                users_json = user_resp.json()
                users_records = users_json.get('records', [])
                all_users = users_records
                while 'nextRecordsUrl' in users_json:
                    next_url = "https://waveinfratech.my.salesforce.com" + users_json['nextRecordsUrl']
                    user_resp = requests.get(next_url, headers=headers, timeout=30)
                    if user_resp.status_code != 200:
                        logger.error(f"Failed to fetch next user page: {user_resp.status_code} - {user_resp.text}")
                        break
                    users_json = user_resp.json()
                    all_users.extend(users_json.get('records', []))
                
                clean_user_records = [{k: v for k, v in record.items() if k != 'attributes'} for record in all_users]
                user_df = pd.DataFrame(clean_user_records)
                user_df = make_arrow_compatible(user_df)
                logger.info(f"Successfully loaded {len(user_df)} users")
                logger.info(f"user_df columns: {list(user_df.columns)}")
                logger.info(f"user_df dtypes:\n{user_df.dtypes}")
            else:
                logger.warning(f"User query failed: {user_resp.status_code}")
                user_df = pd.DataFrame()
        except Exception as e:
